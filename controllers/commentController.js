...
exports.addComment = async (req, res) => {
  ...
  try {
    ...
    // Gửi thông báo cho tác giả manga nếu khác người bình luận
    const Manga = require("../models/manga.model");
    const manga = await Manga.findById(mangaId);
    if (manga?.author?.toString() !== userId.toString()) {
      notificationEmitter.emit("commentReply", {
        senderId: userId,
        receiverId: manga.author,
...
exports.toggleLikeComment = async (req, res) => {
  ...
  const { userId } = req.body;
  try {
    const comment = await require("../models/comment.model").findById(
      commentId
    );
    if (!comment)
      return res
    ...
    const idx = comment.reactions.findIndex(
      (r) => r.userId.toString() === userId.toString() && r.type === "like"
    );
    if (idx === -1) {
      comment.reactions.push({ userId, type: "like" });
...
exports.reportComment = async (req, res) => {
  ...
  try {
    ...
    // Gửi thông báo report cho người tạo comment
    notificationEmitter.emit("contentReported", {
      senderId: userId,
      receiverId: comment.userId,
      commentId,
      reason,
...
exports.addCommentToChapter = async (req, res) => {
  ...
  try {
    ...
    // Gửi thông báo cho author chapter
    const Chapter = require("../models/chapter.model");
    const chapter = await Chapter.findById(chapterId).populate("mangaId");
    const mangaAuthorId = chapter?.mangaId?.author;
    if (mangaAuthorId && mangaAuthorId.toString() !== userId.toString()) {
      notificationEmitter.emit("commentReply", {
        senderId: userId,
        receiverId: mangaAuthorId,
