...
exports.getChapterData = async (req, res, next) => {
  try {
    const { mangaId, chapterNumber } = req.params;
    const data = await chapterService.getChapterData(mangaId, Number(chapterNumber));
    res.json({
      success: true,
      data: {
    ...
    // you can customize status code based on error type
    res.status(404).json({
      success: false,
      message: err.message || "Failed to load chapter.",
    });
  }
};
